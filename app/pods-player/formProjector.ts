import { flatFromFixture, rebuildPayload, type FormField } from "./formMapper";
import {
  AUDITED_FIELD_PRIMITIVE_TYPES,
  normalizeAuditedFieldValue,
...
export class FormProjectionPathError extends Error {
  ...
  constructor(readonly path: string) {
    super(
      `Form projection path is not declared by the active contract: ${path}`,
    );
...
function setAtPath(
  ...
  for (let index = 0; index < parts.length - 1; index += 1) {
    ...
    const next =
      Array.isArray(existing) || isRecord(existing)
        ? existing
        : isIndex(nextPart)
          ? []
          : {};
  ...
  if (Array.isArray(current) && isIndex(last))
    current[Number(last)] = clone(value);
...
function replay(
  ...
  for (const operation of operations) {
    if (operation.type === "set")
      setAtPath(payload, operation.path, operation.value);
    ...
    for (const field of nestedFields) {
      if (field.type === "group" && field.children) {
        if (!field.name || isUiOnlyGroupName(field.name)) {
          if (matches(field.children, prefix, explicitPathsAreAbsolute))
            return true;
          continue;
        }
        ...
        const groupPath = field.path
          ? explicitPathsAreAbsolute
            ? field.path
            : joinPath(prefix, field.path)
          : joinPath(prefix, field.name);
        if (pathsEqual(target, pathParts(groupPath))) return true;
        if (matches(field.children, groupPath, explicitPathsAreAbsolute))
          return true;
        continue;
      }

      if (field.type === "row" && field.fields) {
        if (matches(field.fields, prefix, explicitPathsAreAbsolute))
          return true;
        continue;
      }
      ...
      if (field.type === "repeater" && field.fields && field.name) {
        ...
        if (
          !startsWithParts(target, listParts) ||
          !isIndex(target[listParts.length] ?? "")
        )
          continue;
...
export function applyFormModelOperation(
  ...
  if (operation.type === "set") {
    ...
    if (auditedField) {
      ...
      translated = [
        {
          type: "set",
          path,
          value: normalizeAuditedFieldValue(auditedField, operation.value, {
            path,
          }),
        },
      ];
      ...
      const sentinelPaths = findSentinelPaths(
        rebuildPayload(state.fields, sentinelModel),
      );
      translated = sentinelPaths.map((path) => ({
        type: "set",
        path,
    ...
    translated = findSentinelPaths(sentinelPayload).map((path) => ({
      type: "unset",
      path,
    }));
  ...
  if (translated.length === 0) {
    translated = resolveCanonicalModelPaths(state.fields, operation.path).map(
      (path) =>
        operation.type === "set"
  ...
  if (translated.length === 0) {
    translated = diffValues(
      beforePayload,
      rebuildPayload(state.fields, afterModel),
    );
  }
  ...
  if (isRecord(before) && isRecord(after)) {
    for (const key of new Set([
      ...
      if (!(key in after)) operations.push({ type: "unset", path: childPath });
        ...
        operations.push({ type: "set", path: childPath, value: after[key] });
...
function resolveCanonicalModelPaths(
  fields: FormField[],
  modelPath: string,
): string[] {
    ...
    for (const field of nestedFields) {
      if (field.type === "group" && field.children) {
        if (field.name && !isUiOnlyGroupName(field.name)) {
          ...
          const groupPath = field.path
            ? explicitPathsAreAbsolute
              ? field.path
              : joinPath(payloadPrefix, field.path)
      ...
      if (field.type === "repeater" && field.fields && field.name) {
        ...
        if (remaining.length === 2) paths.push(itemPath);
          ...
          paths.push(
            ...resolve(field.fields, remaining.slice(2), itemPath, false),
          );
        continue;
      }

      if (!field.name || remaining.length !== 1 || remaining[0] !== field.name)
        continue;
...
function findSentinelPaths(
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) =>
      findSentinelPaths(item, path ? `${path}.${index}` : String(index), paths),
    );
...
export function serializeFormProjection(
  state: FormProjectionState,
): Record<string, unknown> {
  ...
  function visit(
    ...
    for (const field of nestedFields) {
      if (field.type === "group" && field.children) {
        ...
        if (value !== undefined)
          setAtPath(target, path, responsiveValue(value, viewport));
      }
    }
  }
...
function isS3KeyLike(value: string): boolean {
  ...
  return Boolean(trimmed) && !/^(https?:|data:|blob:|\/)/.test(trimmed);
}
...
export function normalizePersistedMediaValue(value: unknown): unknown {
  ...
  const s3Key =
    typeof value.s3Key === "string"
      ? value.s3Key.trim()
      : typeof value.src === "string" && isS3KeyLike(value.src)
        ? value.src.trim()
        : "";
  ...
  if (typeof value.decorative === "boolean")
    normalized.decorative = value.decorative;
  ...
  function visit(
    ...
    for (const field of nestedFields) {
      if (field.type === "group" && field.children) {
        ...
        const value = projectionGet(target, path);
        if (value !== undefined)
          setAtPath(target, path, normalizePersistedMediaValue(value));
      }
    }
  }
