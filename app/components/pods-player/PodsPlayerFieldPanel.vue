...
const emit = defineEmits<{
  "update:modelValue": [
    payload: { field: string; value: unknown; structural?: boolean },
  ];
...
const defaultPanelTab = computed<PanelTab>(() =>
  slots.chat ? "chat" : "fields",
);
const activePanelTab = ref<PanelTab>(props.activeTab || defaultPanelTab.value);
const activePodLabel = computed(
  () => props.pod?.label || props.pod?.slug || "Pod",
);
